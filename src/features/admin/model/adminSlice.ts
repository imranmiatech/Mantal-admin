...
export const deleteResearcher = createAsyncThunk(
  'admin/deleteResearcher',
  async (payload: { id: string; page: number; search?: string }, { dispatch }) => {
    const response = await deleteResearcherRequest(payload.id)
    await dispatch(fetchResearchersPage({ page: payload.page, search: payload.search }))
    return response
  },
)
...
export const deletePost = createAsyncThunk(
  'admin/deletePost',
  async (payload: { id: string; page: number }, { dispatch }) => {
    const response = await deletePostRequest(payload.id)
    await dispatch(fetchPublishedSubmissionsPage(payload.page))
    return response
  },
)
